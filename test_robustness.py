...
class HeySnipsNetworkADS(BaseModel):
    def __init__(self,
        ...
        if(os.path.exists(model_path_ads_net)):
            ...
            self.net_discretized_2_bit = NetworkADS.load(model_path_ads_net_2bit)
    ...
    def test(self, data_loader, fn_metrics):
        ...
        for batch_id, [batch, test_logger] in enumerate(data_loader.test_set()):
            ...
            self.net_discretized_2_bit.lyrRes.ts_target = ts_rate_net_target_dynamics
            ...
            val_sim_discretized_2_bit = self.net_discretized_2_bit.evolve(ts_input=ts_spiking_in, verbose=(self.verbose > 1))
            ...
            out_val_discretized_2_bit = val_sim_discretized_2_bit["output_layer"].samples.T
            ...
            self.net_discretized_2_bit.reset_all()
            ...
            # - Compute the final classification output of discretized net
            final_out_discretized_2_bit = out_val_discretized_2_bit.T @ self.w_out
            final_out_discretized_2_bit = filter_1d(final_out_discretized_2_bit, alpha=0.95)
            ...
            # - Check for crossing of the discretized 2bit net
            if ((final_out_discretized_2_bit > self.threshold).any()):
                predicted_label_discretized_2_bit = 1
            else:
                predicted_label_discretized_2_bit = 0
            ...
            if(predicted_label_discretized_2_bit == tgt_label):
                correct_discretized_2_bit += 1
            count += 1
            ...
            if(self.verbose > 0):
                ...
                plt.plot(np.arange(0,len(final_out_discretized_3_bit)*self.dt, self.dt),final_out_discretized_3_bit, label="Discretized 3bit")
                plt.plot(np.arange(0,len(final_out_discretized_2_bit)*self.dt, self.dt),final_out_discretized_2_bit, label="Discretized 2bit")
        ...
        test_acc_discretized_2_bit = correct_discretized_2_bit / count
        test_acc_rate = correct_rate / count
        print("Mismatch 1 test accuracy is %.4f Mismatch 2 test accuracy is %.4f Original test accuracy is %.4f Rate network test accuracy is %.4f Discretized 4bit test accuracy is %.4f Discretized 3bit test accuracy is %.4f Discretized 2bit test accuracy is %.4f" % (test_acc_mismatch_one, test_acc_mismatch_two, test_acc_original, test_acc_rate, test_acc_discretized_4_bit, test_acc_discretized_3_bit, test_acc_discretized_2_bit))
